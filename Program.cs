...
app.MapGet("/api/coordinator/drivers", async (IConfiguration config) => {
    ...
    WHERE d.is_verified = 1 
      AND (d.role = 'SHUTTLE_DRIVER' OR d.role IS NULL OR d.role != 'STUDENT_DRIVER')
    GROUP BY d.driver_id, d.first_name, d.last_name, d.email, d.phone, d.role, d.status, s.student_number;";
...
app.MapPut("/api/coordinator/drivers/{id:int}", async (int id, DriverUpsertDto req, IConfiguration config) => {
    ...
    try
    {
        ...
        string query = @"UPDATE driver 
                         SET first_name = @First, last_name = @Last, email = @Email, status = @Status, phone = @Phone 
                         WHERE driver_id = @Id;";
        ...
        command.Parameters.AddWithValue("@Status", string.IsNullOrEmpty(req.Status) ? "Active" : req.Status);   // NEW LINE
...
app.MapPost("/api/coordinator/schedules", async (ScheduleDirectDto req, IConfiguration config) => {
    ...
    try
    {
        ...
        var timeOfDay = departureDateTime.TimeOfDay;
        if (timeOfDay < TimeSpan.FromHours(6) || timeOfDay > TimeSpan.FromHours(22))
        {
            return Results.BadRequest(new { success = false, message = "Departure time must be between 06:00 and 22:00." });
        }
        ...
        try
        {
            // Insert into trip (drives the Schedule Shuttles list view)
            string tripInsertQuery = @"
        INSERT INTO trip (driver_id, registration_number, departure_stop, destination_stop, 
                           departure_time, trip_type, status)
        VALUES (@DriverID, @RegNumber, @FromStop, @ToStop, @ShiftStart, 'SHUTTLE', 'SCHEDULED');";

            using (var tripCommand = new MySqlCommand(tripInsertQuery, connection, (MySqlTransaction)transaction))
            {
                tripCommand.Parameters.AddWithValue("@DriverID", req.DriverID);
                tripCommand.Parameters.AddWithValue("@RegNumber", regNumber);
                tripCommand.Parameters.AddWithValue("@FromStop", fromLocation);
                tripCommand.Parameters.AddWithValue("@ToStop", toLocation);
                tripCommand.Parameters.AddWithValue("@ShiftStart", departureDateTime);
                await tripCommand.ExecuteNonQueryAsync();
            }

            // Insert into shuttle_assignment (separate tracking table)
            string assignmentInsertQuery = @"
        ...
        VALUES (@RouteID, @DriverID, @RegNumber, @AssignmentDate, @ShiftStart, 
            ...
            using (var assignmentCommand = new MySqlCommand(assignmentInsertQuery, connection, (MySqlTransaction)transaction))
            {
                assignmentCommand.Parameters.AddWithValue("@RouteID", req.RouteID);
                assignmentCommand.Parameters.AddWithValue("@DriverID", req.DriverID);
                assignmentCommand.Parameters.AddWithValue("@RegNumber", regNumber);
                assignmentCommand.Parameters.AddWithValue("@AssignmentDate", dateString);
                assignmentCommand.Parameters.AddWithValue("@ShiftStart", departureDateTime);
        ...
        {
            await transaction.RollbackAsync();
            throw; // re-thrown so your existing outer catch block still logs and returns the 500 error
        }
